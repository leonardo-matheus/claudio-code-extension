...
</html>`}};var B=k(require("vscode")),Y=k(require("https")),Z=k(require("http")),X=2e5,Q=[{name:"Bash",description:"Execute shell command. Server commands run in background automatically.",input_schema:{type:"object",properties:{command:{type:"string",description:"Command to execute"},workingDirectory:{type:"string",description:"Working directory (optional)"},background:{type:"boolean",description:"Force background execution"}},required:["command"]}},{name:"Read",description:"Read file contents",input_schema:{type:"object",properties:{path:{type:"string",description:"File path"},startLine:{type:"number",description:"Start line (optional)"},endLine:{type:"number",description:"End line (optional)"}},required:["path"]}},{name:"Write",description:"Create or overwrite a file",input_schema:{type:"object",properties:{path:{type:"string",description:"File path"},content:{type:"string",description:"File content"}},required:["path","content"]}},{name:"Edit",description:"Edit file by replacing text (oldText must be unique)",input_schema:{type:"object",properties:{path:{type:"string",description:"File path"},oldText:{type:"string",description:"Text to replace"},newText:{type:"string",description:"New text"}},required:["path","oldText","newText"]}},{name:"ListDir",description:"List files and folders",input_schema:{type:"object",properties:{path:{type:"string",description:"Directory path"},recursive:{type:"boolean",description:"List recursively"}},required:["path"]}},{name:"Search",description:"Search for pattern in files",input_schema:{type:"object",properties:{pattern:{type:"string",description:"Search pattern"},path:{type:"string",description:"Directory (optional)"}},required:["pattern"]}},{name:"Glob",description:"Find files by glob pattern",input_schema:{type:"object",properties:{pattern:{type:"string",description:"Glob pattern (e.g., **/*.ts)"}},required:["pattern"]}}],L=class{constructor(){this.messages=[];this.currentTokens={input:0,output:0};this.abortController=null;this.isProcessing=!1}getConfig(){let t=B.workspace.getConfiguration("claudioCode");return{apiKey:t.get("apiKey")||"",apiUrl:t.get("apiUrl")||"https://claudioai.dev",model:t.get("model")||"claude-opus-4-5",maxTokens:t.get("maxTokens")||8192}}getModel(){return this.getConfig().model}clearHistory(){this.messages=[],this.currentTokens={input:0,output:0}}getMessages(){return[...this.messages]}setMessages(t){this.messages=[...t]}getTokenUsage(){return{inputTokens:this.currentTokens.input,outputTokens:this.currentTokens.output,totalTokens:this.currentTokens.input+this.currentTokens.output,percentUsed:Math.round(this.currentTokens.input/X*100)}}setTokens(t,e){this.currentTokens={input:t,output:e}}abort(){return this.abortController&&this.isProcessing?(this.abortController.abort(),this.isProcessing=!1,!0):!1}getIsProcessing(){return this.isProcessing}getSystemPrompt(){return`You are Claudio Code, an advanced AI coding assistant.
...
6. Respond in the user's language`}async sendMessage(t,e,n){let s=this.getConfig();if(!s.apiKey)throw new Error("API Key not configured. Go to Settings > Claudio Code > API Key");this.abortController=new AbortController,this.isProcessing=!0;let o=this.buildUserContent(t,e);this.messages.push({role:"user",content:o});let i=0,r=25,p="";for(;i<r;){if(this.abortController?.signal.aborted)return this.isProcessing=!1,p||"Stopped by user";i++;try{let a=await this.streamRequest(s,n);if(p=a.text,!a.hasToolCalls)break}catch(a){throw this.onError?.(a.message),this.isProcessing=!1,a}}return this.isProcessing=!1,p}buildUserContent(t,e){if(!e?.length)return t;let n=[];for(let s of e)n.push({type:"image",source:{type:"base64",media_type:s.type,data:s.data}});return n.push({type:"text",text:t}),n}async streamRequest(t,e){return new Promise((n,s)=>{let o=JSON.stringify({model:t.model,max_tokens:t.maxTokens,stream:!0,system:this.getSystemPrompt(),tools:Q,messages:this.messages}),i=new URL(`${t.apiUrl}/v1/messages`),r=i.protocol==="https:",a=(r?Y:Z).request({hostname:i.hostname,port:i.port||(r?443:80),path:i.pathname,method:"POST",headers:{"Content-Type":"application/json","x-api-key":t.apiKey,"anthropic-version":"2023-06-01"}},async c=>{if(c.statusCode!==200){let T="";c.on("data",g=>T+=g),c.on("end",()=>{try{let g=JSON.parse(T);s(new Error(g.error?.message||`HTTP ${c.statusCode}`))}catch{s(new Error(`HTTP ${c.statusCode}: ${T.substring(0,200)}`))}});return}let v="",u="",m="",y=[],E=[],$=!1,j=!1;c.on("data",T=>{v+=T.toString();let g=v.split(`
`);v=g.pop()||"";for(let f of g){if(f.startsWith("event:")||f.trim()===""||!f.startsWith("data:"))continue;let F=f.slice(5).trim();if(!(F==="[DONE]"||!F))try{let h=JSON.parse(F);switch(h.type){case"content_block_start":h.content_block?.type==="thinking"?($=!0,this.onThinkingStart?.()):h.content_block?.type==="text"?(j=!0,this.onTextStart?.()):h.content_block?.type==="tool_use"&&y.push({type:"tool_use",id:h.content_block.id,name:h.content_block.name,input:{}});break;case"content_block_delta":if(h.delta?.type==="thinking_delta")m+=h.delta.thinking,this.onThinkingUpdate?.(h.delta.thinking);else if(h.delta?.type==="text_delta")u+=h.delta.text,this.onTextDelta?.(h.delta.text);else if(h.delta?.type==="input_json_delta"&&y.length>0){let C=y[y.length-1];C._inputJson||(C._inputJson=""),C._inputJson+=h.delta.partial_json}break;case"content_block_stop":if($)$=!1,this.onThinkingEnd?.(),m&&E.push({type:"thinking",text:m}),m="";else if(j)j=!1,this.onTextEnd?.(),u&&E.push({type:"text",text:u});else if(y.length>0){let C=y[y.length-1];if(C._inputJson){try{C.input=JSON.parse(C._inputJson)}catch{}delete C._inputJson}E.push(C)}break;case"message_delta":h.usage&&(this.currentTokens.input=h.usage.input_tokens||this.currentTokens.input,this.currentTokens.output=h.usage.output_tokens||this.currentTokens.output,this.onTokenUpdate?.(this.getTokenUsage()));break;case"message_stop":break}}catch{}}}),c.on("end",async()=>{if(E.length>0&&this.messages.push({role:"assistant",content:E}),y.length>0&&e){let T=[];for(let g of y){this.onToolStart?.(g.name,g.input);try{let f=await e.execute(g.name,g.input);this.onToolEnd?.(g.name,f,!0),T.push({type:"tool_result",tool_use_id:g.id,content:f.length>1e4?f.substring(0,1e4)+`
...(truncated)`:f})}catch(f){this.onToolEnd?.(g.name,f.message,!1),T.push({type:"tool_result",tool_use_id:g.id,content:`Error: ${f.message}`,is_error:!0})}}this.messages.push({role:"user",content:T}),n({text:u,hasToolCalls:!0})}else n({text:u,hasToolCalls:!1})}),c.on("error",s)});a.on("error",s),a.setTimeout(12e4,()=>{a.destroy(),s(new Error("Request timeout"))}),a.write(o),a.end()})}};var x=k(require("vscode")),w=k(require("path")),l=k(require("fs")),O=require("child_process"),q=require("util"),tt=(0,q.promisify)(O.exec),I=new Set(["node_modules",".git","__pycache__",".venv","venv","dist","build",".next",".nuxt","vendor","target"]),A=class{constructor(){this.bypassPermissions=!1}setBypassPermissions(t){this.bypassPermissions=t}getWorkspacePath(){return x.workspace.workspaceFolders?.[0]?.uri.fsPath||process.cwd()}resolvePath(t){return w.isAbsolute(t)?t:w.join(this.getWorkspacePath(),t)}async execute(t,e){switch(t){case"Bash":return this.executeBash(e);case"Read":return this.readFile(e);case"Write":return this.writeFile(e);case"Edit":return this.editFile(e);case"ListDir":return this.listDirectory(e);case"Search":return this.search(e);case"Glob":return this.glob(e);default:throw new Error(`Unknown tool: ${t}`)}}async executeBash(t){let{command:e,workingDirectory:n}=t,s=n?this.resolvePath(n):this.getWorkspacePath(),o=[/npm\s+(run\s+)?(start|dev|serve)/i,/yarn\s+(start|dev)/i,/vite/i];if(t.background||o.some(a=>a.test(e))){let a=x.window.createTerminal({name:e.substring(0,30),cwd:s});return a.sendText(e),a.show(),`Started in terminal: ${e}`}if([/rm\s+-rf\s+[/~]/,/sudo/,/chmod\s+777/].some(a=>a.test(e))&&!this.bypassPermissions&&await x.window.showWarningMessage(`Dangerous command: ${e}`,{modal:!0},"Execute","Cancel")!=="Execute")return"Cancelled by user";let p=["ls","cat","pwd","git status","git log","git diff","npm list","echo","which"].some(a=>e.trim().startsWith(a));if(!this.bypassPermissions&&!p&&await x.window.showInformationMessage(`Execute: ${e.substring(0,80)}...`,"Execute","Cancel")!=="Execute")return"Cancelled by user";try{let{stdout:a,stderr:c}=await tt(e,{cwd:s,timeout:6e4,maxBuffer:5242880,shell:process.platform==="win32"?"cmd.exe":"/bin/bash"});return(a+c).trim()||"Command executed successfully"}catch(a){throw new Error(a.message)}}async readFile(t){let e=this.resolvePath(t.path);if(!l.existsSync(e))throw new Error(`File not found: ${t.path}`);let n=l.statSync(e);if(n.isDirectory())throw new Error(`Path is a directory: ${t.path}`);if(n.size>1024*1024)throw new Error(`File too large (${(n.size/1024/1024).toFixed(1)}MB)`);let o=l.readFileSync(e,"utf-8").split(`
`);if(t.startLine||t.endLine){let i=Math.max(0,(t.startLine||1)-1),r=Math.min(o.length,t.endLine||o.length);return o.slice(i,r).map((p,a)=>`${i+a+1}\u2502 ${p}`).join(`
...
`):"No matches found"}glob(t){let e=this.getWorkspacePath(),n=new RegExp(t.pattern.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*\*/g,".*").replace(/\*/g,"[^/]*")),s=[],o=(i,r)=>{if(!(s.length>=50))try{let p=l.readdirSync(i);for(let a of p){if(I.has(a))continue;let c=w.join(i,a),v=r?`${r}/${a}`:a;l.statSync(c).isDirectory()?o(c,v):n.test(v)&&s.push(v)}}catch{}};return o(e,""),s.length>0?s.join(`
`):"No files found"}};var z;function et(d){console.log("Claudio Code: Activating...");let t=new L,e=new A;z=new D(d.extensionUri,t,e,d),d.subscriptions.push(b.window.registerWebviewViewProvider("claudio-code.chatView",z,{webviewOptions:{retainContextWhenHidden:!0}}),b.commands.registerCommand("claudio-code.openChat",()=>{b.commands.executeCommand("claudio-code.chatView.focus")}),b.commands.registerCommand("claudio-code.newChat",()=>{z.newChat()}),b.commands.registerCommand("claudio-code.openSettings",()=>{b.commands.executeCommand("workbench.action.openSettings","claudioCode")})),b.workspace.getConfiguration("claudioCode").get("apiKey")||b.window.showWarningMessage("Claudio Code: Configure your API Key in settings.","Open Settings").then(s=>{s==="Open Settings"&&b.commands.executeCommand("workbench.action.openSettings","claudioCode.apiKey")}),console.log("Claudio Code: Activated")}function st(){console.log("Claudio Code: Deactivated")}0&&(module.exports={activate,deactivate});
