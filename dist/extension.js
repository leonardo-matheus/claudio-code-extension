...
<div class="no-key-icon">\u{1F511}</div>
...
</html>`}};var U=C(require("vscode")),X=C(require("https")),Q=C(require("http")),ee=2e5,te=[{name:"Bash",description:"Execute shell command. Server commands run in background automatically.",input_schema:{type:"object",properties:{command:{type:"string",description:"Command to execute"},workingDirectory:{type:"string",description:"Working directory (optional)"},background:{type:"boolean",description:"Force background execution"}},required:["command"]}},{name:"Read",description:"Read file contents",input_schema:{type:"object",properties:{path:{type:"string",description:"File path"},startLine:{type:"number",description:"Start line (optional)"},endLine:{type:"number",description:"End line (optional)"}},required:["path"]}},{name:"Write",description:"Create or overwrite a file",input_schema:{type:"object",properties:{path:{type:"string",description:"File path"},content:{type:"string",description:"File content"}},required:["path","content"]}},{name:"Edit",description:"Edit file by replacing text (oldText must be unique)",input_schema:{type:"object",properties:{path:{type:"string",description:"File path"},oldText:{type:"string",description:"Text to replace"},newText:{type:"string",description:"New text"}},required:["path","oldText","newText"]}},{name:"ListDir",description:"List files and folders",input_schema:{type:"object",properties:{path:{type:"string",description:"Directory path"},recursive:{type:"boolean",description:"List recursively"}},required:["path"]}},{name:"Search",description:"Search for pattern in files",input_schema:{type:"object",properties:{pattern:{type:"string",description:"Search pattern"},path:{type:"string",description:"Directory (optional)"}},required:["pattern"]}},{name:"Glob",description:"Find files by glob pattern",input_schema:{type:"object",properties:{pattern:{type:"string",description:"Glob pattern (e.g., **/*.ts)"}},required:["pattern"]}}],A=class{constructor(){this.messages=[];this.currentTokens={input:0,output:0};this.abortController=null;this.isProcessing=!1}getConfig(){let e=U.workspace.getConfiguration("claudioCode");return{apiKey:e.get("apiKey")||"",apiUrl:e.get("apiUrl")||"https://claudioai.dev",model:e.get("model")||"o1",maxTokens:e.get("maxTokens")||8192}}getModel(){return this.getConfig().model}clearHistory(){this.messages=[],this.currentTokens={input:0,output:0}}getMessages(){return[...this.messages]}setMessages(e){this.messages=[...e]}getTokenUsage(){return{inputTokens:this.currentTokens.input,outputTokens:this.currentTokens.output,totalTokens:this.currentTokens.input+this.currentTokens.output,percentUsed:Math.round(this.currentTokens.input/ee*100)}}setTokens(e,t){this.currentTokens={input:e,output:t}}abort(){return this.abortController&&this.isProcessing?(this.abortController.abort(),this.isProcessing=!1,!0):!1}getIsProcessing(){return this.isProcessing}getSystemPrompt(){return`You are Claudio Code, an advanced AI coding assistant.
...
## Working Directory
${U.workspace.workspaceFolders?.[0]?.uri.fsPath||process.cwd()}
...
6. Respond in the user's language`}async sendMessage(e,t,n){let s=this.getConfig();if(!s.apiKey)throw new Error("API Key not configured. Go to Settings > Claudio Code > API Key");this.abortController=new AbortController,this.isProcessing=!0;let o=this.buildUserContent(e,t);this.messages.push({role:"user",content:o});let i=0,r=25,g="";for(;i<r;){if(this.abortController?.signal.aborted)return this.isProcessing=!1,g||"Stopped by user";i++;try{let a=await this.streamRequest(s,n);if(g=a.text,!a.hasToolCalls)break}catch(a){throw this.onError?.(a.message),this.isProcessing=!1,a}}return this.isProcessing=!1,g}buildUserContent(e,t){if(!t?.length)return e;let n=[];for(let s of t)n.push({type:"image",source:{type:"base64",media_type:s.type,data:s.data}});return n.push({type:"text",text:e}),n}async streamRequest(e,t){return new Promise((n,s)=>{let o=JSON.stringify({model:e.model,max_tokens:e.maxTokens,stream:!0,system:this.getSystemPrompt(),tools:te,messages:this.messages}),i=new URL(`${e.apiUrl}/v1/messages`),r=i.protocol==="https:",a=(r?X:Q).request({hostname:i.hostname,port:i.port||(r?443:80),path:i.pathname,method:"POST",headers:{"Content-Type":"application/json","x-api-key":e.apiKey,"anthropic-version":"2023-06-01"}},async c=>{if(c.statusCode!==200){let S="";c.on("data",u=>S+=u),c.on("end",()=>{try{let u=JSON.parse(S);s(new Error(u.error?.message||`HTTP ${c.statusCode}`))}catch{s(new Error(`HTTP ${c.statusCode}: ${S.substring(0,200)}`))}});return}let w="",f="",x="",k=[],M=[],j=!1,B=!1,E=0,P=0;c.on("data",S=>{w+=S.toString();let u=w.split(`
`);w=u.pop()||"";for(let b of u){if(b.startsWith("event:")||b.trim()===""||!b.startsWith("data:"))continue;let I=b.slice(5).trim();if(!(I==="[DONE]"||!I))try{let p=JSON.parse(I);switch(p.type){case"content_block_start":p.content_block?.type==="thinking"?(j=!0,this.onThinkingStart?.()):p.content_block?.type==="text"?(B=!0,this.onTextStart?.()):p.content_block?.type==="tool_use"&&k.push({type:"tool_use",id:p.content_block.id,name:p.content_block.name,input:{}});break;case"message_start":p.message?.usage?.input_tokens&&(this.currentTokens.input=p.message.usage.input_tokens,this.onTokenUpdate?.(this.getTokenUsage()));break;case"content_block_delta":if(p.delta?.type==="thinking_delta"){x+=p.delta.thinking,E+=p.delta.thinking?.length||0,this.onThinkingUpdate?.(p.delta.thinking);let m=Date.now();m-P>500&&(this.currentTokens.output=Math.round(E/4),this.onTokenUpdate?.(this.getTokenUsage()),P=m)}else if(p.delta?.type==="text_delta"){f+=p.delta.text,E+=p.delta.text?.length||0,this.onTextDelta?.(p.delta.text);let m=Date.now();m-P>500&&(this.currentTokens.output=Math.round(E/4),this.onTokenUpdate?.(this.getTokenUsage()),P=m)}else if(p.delta?.type==="input_json_delta"&&k.length>0){let m=k[k.length-1];m._inputJson||(m._inputJson=""),m._inputJson+=p.delta.partial_json}break;case"content_block_stop":if(j)j=!1,this.onThinkingEnd?.(),x&&M.push({type:"thinking",text:x}),x="";else if(B)B=!1,this.onTextEnd?.(),f&&M.push({type:"text",text:f});else if(k.length>0){let m=k[k.length-1];if(m._inputJson){try{m.input=JSON.parse(m._inputJson)}catch{}delete m._inputJson}M.push(m)}break;case"message_delta":p.usage&&(this.currentTokens.input=p.usage.input_tokens||this.currentTokens.input,this.currentTokens.output=p.usage.output_tokens||this.currentTokens.output,this.onTokenUpdate?.(this.getTokenUsage()));break;case"message_stop":break}}catch{}}}),c.on("end",async()=>{if(M.length>0&&this.messages.push({role:"assistant",content:M}),k.length>0&&t){let S=[];for(let u of k){this.onToolStart?.(u.name,u.input);try{let b=await t.execute(u.name,u.input);this.onToolEnd?.(u.name,b,!0),S.push({type:"tool_result",tool_use_id:u.id,content:b.length>1e4?b.substring(0,1e4)+`
...(truncated)`:b})}catch(b){this.onToolEnd?.(u.name,b.message,!1),S.push({type:"tool_result",tool_use_id:u.id,content:`Error: ${b.message}`,is_error:!0})}}this.messages.push({role:"user",content:S}),n({text:f,hasToolCalls:!0})}else n({text:f,hasToolCalls:!1})}),c.on("error",s)});a.on("error",s),a.setTimeout(12e4,()=>{a.destroy(),s(new Error("Request timeout"))}),a.write(o),a.end()})}};var v=C(require("vscode")),T=C(require("path")),l=C(require("fs")),H=require("child_process"),K=require("util"),se=(0,K.promisify)(H.exec),z=new Set(["node_modules",".git","__pycache__",".venv","venv","dist","build",".next",".nuxt","vendor","target"]),F=class{constructor(){this.bypassPermissions=!1}setBypassPermissions(e){this.bypassPermissions=e}getWorkspacePath(){return v.workspace.workspaceFolders?.[0]?.uri.fsPath||process.cwd()}resolvePath(e){return T.isAbsolute(e)?e:T.join(this.getWorkspacePath(),e)}async execute(e,t){switch(e){case"Bash":return this.executeBash(t);case"Read":return this.readFile(t);case"Write":return this.writeFile(t);case"Edit":return this.editFile(t);case"ListDir":return this.listDirectory(t);case"Search":return this.search(t);case"Glob":return this.glob(t);default:throw new Error(`Unknown tool: ${e}`)}}async executeBash(e){let{command:t,workingDirectory:n}=e,s=n?this.resolvePath(n):this.getWorkspacePath(),o=[/npm\s+(run\s+)?(start|dev|serve)/i,/yarn\s+(start|dev)/i,/vite/i];if(e.background||o.some(a=>a.test(t))){let a=v.window.createTerminal({name:t.substring(0,30),cwd:s});return a.sendText(t),a.show(),`Started in terminal: ${t}`}if([/rm\s+-rf\s+[/~]/,/sudo/,/chmod\s+777/].some(a=>a.test(t))&&!this.bypassPermissions&&await v.window.showWarningMessage(`Dangerous command: ${t}`,{modal:!0},"Execute","Cancel")!=="Execute")return"Cancelled by user";let g=["ls","cat","pwd","git status","git log","git diff","npm list","echo","which"].some(a=>t.trim().startsWith(a));if(!this.bypassPermissions&&!g&&await v.window.showInformationMessage(`Execute: ${t.substring(0,80)}...`,"Execute","Cancel")!=="Execute")return"Cancelled by user";try{let{stdout:a,stderr:c}=await se(t,{cwd:s,timeout:6e4,maxBuffer:5242880,shell:process.platform==="win32"?"cmd.exe":"/bin/bash"});return(a+c).trim()||"Command executed successfully"}catch(a){throw new Error(a.message)}}async readFile(e){let t=this.resolvePath(e.path);if(!l.existsSync(t))throw new Error(`File not found: ${e.path}`);let n=l.statSync(t);if(n.isDirectory())throw new Error(`Path is a directory: ${e.path}`);if(n.size>1024*1024)throw new Error(`File too large (${(n.size/1024/1024).toFixed(1)}MB)`);let o=l.readFileSync(t,"utf-8").split(`
